...
Regression::Regression(const Configuration& config) : TataBase(config) {
...
Bucket Regression::run(Bucket& bucket, RegTask reg_task) {
	...
	if (reg_task == RegTask::LUNG_VOLUME) {
		...
		std::string model_path = m_config.weight_path + m_config.LungRegression.weight_name;
